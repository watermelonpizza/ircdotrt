...
namespace IrcDotRT
{
    ...
    public partial class IrcClient : INotifyPropertyChanged, IDisposable
    {
        ...
        private IrcDotRT.Collections.ReadOnlyDictionary<string, string> serverSupportedFeaturesReadOnly;
        ...
        public IrcClient()
        {
            ...
            sendTimer = new Timer(new TimerCallback(WritePendingMessages), null, Timeout.Infinite, Timeout.Infinite);
            ...
            readRunner = new Action(ReceiveAsync);
        ...
        public IrcDotRT.Collections.ReadOnlyDictionary<string, string> ServerSupportedFeatures
        {
            get { return serverSupportedFeaturesReadOnly; }
        }
        ...
        {
            get { return channelUserModesReadOnly; }
        }
        ...
        protected bool HandleISupportParameter(string paramName, string paramValue)
        {
            ...
            switch (paramName.ToLowerInvariant())
            {
                case "prefix":
                    ...
                    lock (((ICollection)channelUserModesReadOnly).SyncRoot)
                    {
        ...
        protected Tuple<string, ReadOnlyCollection<string>> GetModeAndParameters(IEnumerable<string> messageParameters)
        {
            ...
            return Tuple.Create(modes.ToString(), new ReadOnlyCollection<string>(modeParameters));
        }
        ...
        protected IIrcMessageSource GetSourceFromPrefix(string prefix)
        {
            ...
            {
                Assembly s = typeof(IrcClient).GetTypeInfo().Assembly;
                throw new ArgumentException(string.Format(
        ...
        protected IrcChannel GetChannelFromName(string channelName, out bool createdNew)
        {
            ...
            lock (((ICollection)channelsReadOnly).SyncRoot)
            {
        ...
        private void ResetState()
        {
            ...
            serverSupportedFeaturesReadOnly = new IrcDotRT.Collections.ReadOnlyDictionary<string, string>(serverSupportedFeatures);
            channelUserModes = new Collection<char>() {
                'o', 'v' };
            channelUserModesReadOnly = new ReadOnlyCollection<char>(channelUserModes);
            ...
            channelsReadOnly = new IrcChannelCollection(this, channels);
            users = new Collection<IrcUser>();
            usersReadOnly = new IrcUserCollection(this, users);
        ...
        private void WritePendingMessages(object state)
        {
            try
            {
                ...
                sendTimer.Change(Convert.ToInt32(Math.Max(sendDelay, minimumSendWaitTime)), Timeout.Infinite);
            }
        ...
        /// <param name="port">The port number of the remote host.</param>
        private void Connect(HostName address, int port, bool useSsl, IrcRegistrationInfo registrationInfo)
        {
            CheckDisposed();
            ...
            Connect(new EndpointPair(null, string.Empty, address, port.ToString()), useSsl, registrationInfo);
        }
        ...
        /// <exception cref="ObjectDisposedException">The current instance has already been disposed.</exception>
        private void Connect(EndpointPair remoteEndPoint, bool useSsl, IrcRegistrationInfo registrationInfo)
        {
            CheckDisposed();
        ...
        private async void SendAsync(byte[] buffer, int offset, int count, object token = null)
        {
            ...
            OnRawMessageSent((IrcRawMessageEventArgs)token);
        }
        ...
        private async void ConnectAsync(EndpointPair remoteEndPoint, object token = null)
        {
            try
            {
                ...
                HandleClientConnected(obj.Item3);
            }
            catch (ObjectDisposedException)
            {
                // Ignore.
        ...
        {
            DebugUtilities.WriteEvent(string.Format("Connected to server at '{0}'.", socket.Information.RemoteAddress.DisplayName));
